...
public class TestContextPerformance extends AbstractContextTester {
	...
	public TestContextPerformance() {
		super();
		// TODO Auto-generated constructor stub
	}

	public TestContextPerformance(String arg0) {
		super(arg0);
		// TODO Auto-generated constructor stub
	}
	...
	public void testTheConcurrencyChallenge() throws Exception {
		...
		for (int i = 0; i < 80000; i += 8) {
			...
			Properties propsb = (Properties) ((Future) results.get(i + 5))
					.get();
			assertEquals("Failed at i = " + i, "red", propsa
					.getProperty("color"));
			assertEquals("Failed at i = " + i, "purple", propsb
					.getProperty("color"));
			if (ms == null) {
			...
			assertEquals(ms,
					((Future<BasicContextOriented>) results.get(i + 1)).get()
							.getMyService());
			assertEquals("Failed at i = " + i, "path1",
					((Future<BasicContextOriented>) results.get(i + 1)).get()
							.getPaths()[0]);
			assertEquals("Failed at i = " + i, "orc_path1",
					((Future<BasicContextOriented>) results.get(i + 2)).get()
							.getPaths()[0]);
			assertEquals("Failed at i = " + i, ms,
					((Future<BasicContextOriented>) results.get(i + 2)).get()
							.getMyService());
			assertEquals("Failed at i = " + i, "custom/units/",
	...
	public void testRefreshMemoryPerformance() throws Exception {
		...
		for (int i = 0; i < 1000; i++) {
			...
			if (memUsed == 0) {
				...
				memory.gc();
				Thread.sleep(20);
				assertTrue("Some memory leakage on iteration " + i
						+ " memUsed =  " + memUsed + " current mem = "
						+ memory.getHeapMemoryUsage().getUsed(),
						memUsed > memory.getHeapMemoryUsage().getUsed());
			}
		}
